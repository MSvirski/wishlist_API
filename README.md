...
На данный момент представляет собой микросервис авторизации и аутентификации пользователей. Позволяет добавить желаемый товар. 

## 🚀 Стек технологий
* **Backend:** Python 3.11, FastAPI, Pydantic v2
* **Database:** PostgreSQL 15, SQLAlchemy (ORM)
* **Security:** JWT (JSON Web Tokens), PyJWT, Bcrypt
...
* **Изоляция сред (Dev / Test):** В Docker-окружении развернуты два независимых контейнера PostgreSQL. Основная база данных (`db`) использует именованный том (`postgres_data`) для постоянного хранения данных. Тестовая база (`db-test`) полностью изолирована и уничтожается при остановке контейнеров.
...
│   │    ├── dependencies.py       # Описание зависимостей(проверка JWT)
│   │    ├── models.py             # Описание таблиц auth базы данных  (SQLAlchemy ORM)
│   │    ├── router.py             # Эндпоинты сервера и роутинг API
│   │    ├── schemas.py            # Схемы валидации данных (Pydantic v2)
│   │    └── security.py           # Логика хэширования паролей и генерации JWT
│   ├── wishlist/                 # Список желаний
│       ├── models.py             # Описание таблиц wishlist базы данных  (SQLAlchemy ORM)
│       ├── router.py             # эндпоинты сервера и роутинг API
│       ├── schemas.py            # Схемы валидации данных (Pydantic v2)
│       └── service.py            # Логика получения данных из бд
├── .env.example              # Пример файла с переменными окружения
├── .gitignore                # Файл исключений для системы контроля версий Git
├── autorun.sh                # Скрипт для быстрой автоматической сборки проекта
├── Dockerfile                # Инструкция для сборки Docker-образа приложения
├── docker-compose.yml        # Оркестрация контейнеров (FastAPI + PostgreSQL)
├── pyproject.toml            # Настройки сторонних библиотек
